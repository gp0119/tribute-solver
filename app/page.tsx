      ...
      <header className="topbar">
        ...
        <div className="header-actions">
          <nav className="page-nav" aria-label="页面导航">
            <Link className="page-link active" href="/">推演器</Link>
            <Link className="page-link" href="/practice">做题</Link>
          </nav>
          <span className="save-status"><i aria-hidden="true" />本机保存</span>
          <button type="button" className="quiet-button" onClick={reset}>新一题</button>
        </div>
      </header>

      <section className="notice" aria-label="使用说明">
        <span className="notice-mark">!</span>
        <p><strong>◎</strong> 是颜色和位置都对，<strong>◉</strong> 是颜色对但位置错；结果只表示数量，不对应单独某格。</p>
      </section>

      <section className="history-section history-section-top" aria-labelledby="history-title">
        <div className="history-heading">
          <div><p className="section-kicker">已记录</p><h2 id="history-title">推理记录</h2></div>
          ...
          <ol className="history-list">
            {attempts.map((attempt, index) => (
              <li key={attempt.id}>
                ...
                <span className="result-summary exact" title="完全正确"><b className="result-icon exact">◎</b><strong>{attempt.exact}</strong></span>
                <span className="result-summary misplaced" title="颜色正确但位置错误"><b className="result-icon misplaced">◉</b><strong>{attempt.misplaced}</strong></span>
                <button type="button" className="delete-button" onClick={() => saveAttempts(attempts.filter((item) => item.id !== attempt.id))} aria-label={`删除第${index + 1}行`}>删除</button>
              </li>
            ))}
          </ol>
        )}
      ...
      <div className="workspace-grid">
        <section className="panel entry-panel" aria-labelledby="entry-title">
          <div className="panel-heading">
            <div><p className="section-kicker">录入本行</p><h2 id="entry-title">第 {Math.min(attempts.length + 1, MAX_RECORDED_ATTEMPTS)} 行</h2></div>
            <span className="row-counter">{attempts.length}/{MAX_RECORDED_ATTEMPTS}</span>
          </div>
                  ...
                  <div className="color-options practice-color-options">
                    {COLORS.map((color) => (
                      <button
                        ...
                        style={{ '--swatch': color.swatch, '--edge': color.edge, '--sprite-tile-x': color.spriteTileX } as CSSProperties}
                        aria-pressed={selectedColor === color.id}
                        onClick={() => setDraftGuess((current) => current.map((value, index) => (index === position ? color.id : value)) as ColorId[])}
                      >
                        <span aria-hidden="true" /><b>{color.name}</b>
                      </button>
                    ))}
                  </div>
                </div>
            ...
            <div className="score-controls">
                ...
                <div className="selected-feedback">
                  ...
                  <div className="feedback-input-row">
                    <div className="feedback-result exact" aria-label={`完全正确 ${draftScore.exact} 个`}>
                      <span className="result-icon-list" aria-hidden="true">
                        {Array.from({ length: draftScore.exact }, (_, index) => <b className="result-icon exact" key={index}>◎</b>)}
                      </span>
                    </div>
                  ...
                  <div className="feedback-input-row">
                    <div className="feedback-result misplaced" aria-label={`颜色对位置错 ${draftScore.misplaced} 个`}>
                      <span className="result-icon-list" aria-hidden="true">
                        {Array.from({ length: draftScore.misplaced }, (_, index) => <b className="result-icon misplaced" key={index}>◉</b>)}
                      </span>
                    </div>
                    <CountOptions label="颜色对位置错数量" value={draftScore.misplaced} onChange={(value) => setScore('misplaced', value)} tone="misplaced" maximum={4 - draftScore.exact} />
                  </div>
                </div>
              </div>
              <button type="button" className="primary-button" onClick={addAttempt} disabled={tooManyRows}>记录这一行</button>
              ...
              {candidates.length > 1 && recommendation && (
                <div className="recommendation-card sidebar-recommendation">
                  ...
                  <button type="button" className="secondary-button" onClick={() => setDraftGuess([...recommendation.code])}>用作当前颜色</button>
                </div>
              )}
            </div>
          </div>
        ...
        <section className="panel analysis-panel" aria-labelledby="analysis-title">
          <div className="panel-heading">
            <div><p className="section-kicker">即时推断</p><h2 id="analysis-title">{candidates.length === 0 ? '记录有矛盾' : isSolved ? '答案已确定' : '剩余候选'}</h2></div>
            <strong className={candidates.length === 0 ? 'candidate-count error' : 'candidate-count'}>{candidates.length}</strong>
          </div>

          {candidates.length === 0 ? (
            <div className="empty-state"><p>没有任何排列符合这些结果。请检查某一行的两种图标数量，或删除该行后重填。</p></div>
          ) : isSolved ? (
            <div className="solution-card"><p>最终答案</p><CodeChips code={candidates[0]} /></div>
          ) : null}

          {candidates.length > 1 && candidates.length <= 24 && (
            <div className="candidate-list" aria-label="全部剩余候选">
              <div className="candidate-list-heading"><p>候选清单</p><span>{candidates.length} 组</span></div>
              <div className="candidate-grid">{candidates.map((code) => <CodeChips code={code} small key={codeKey(code)} />)}</div>
            </div>
          )}
